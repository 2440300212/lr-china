...
namespace GagongXerath
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Base menu
            Menu = new Menu("远古巫灵-泽拉斯", "GagongXerath", true);
            ...
            //Orbwalker
            orbwalkerMenu.AddItem(new MenuItem("Orbwalker_Mode", "走砍规则").SetValue(false));
            ...
            Menu.SubMenu("Harass").AddItem(new MenuItem("HarassAAQ", "Q骚扰接AA").SetValue(false));
            Menu.SubMenu("Harass").AddItem(new MenuItem("HarassTurret", "敌方塔内侧停止骚扰").SetValue(false));
            ...
            Menu.SubMenu("Harass").AddItem(new MenuItem("HarassMana", "骚扰 魔法值  >").SetValue(new Slider(0, 0, 100)));
            ...
            //JungleFarm menu:
            Menu.AddSubMenu(new Menu("打野选项", "JungleFarm"));
            ...
            Menu.SubMenu("AutoKS").AddItem(new MenuItem("UseFK2", "闪现DFG+R").SetValue(true));
            Menu.SubMenu("AutoKS").AddSubMenu(new Menu("使用闪现Kill", "FKT"));
            ...
            Menu.SubMenu("AutoKS").AddItem(new MenuItem("FKMANA", "蓝量足够时闪现 > FC").SetValue(false));
            
            //Misc
            Menu.AddSubMenu(new Menu("其他选项", "Misc"));
            Menu.SubMenu("Misc").AddItem(new MenuItem("AntiGap", "防近身").SetValue(true));
            ...
            Menu.SubMenu("Misc").AddItem(new MenuItem("IgniteALLCD", "自动点燃").SetValue(false));
            if (Menu.Item("Orbwalker_Mode").GetValue<bool>()) Menu.SubMenu("Misc").AddItem(new MenuItem("OrbWAA", "走砍接AA").SetValue(true));
            ...
            Menu.SubMenu("Misc").AddItem(new MenuItem("YasuoWall", "避开亚索风墙").SetValue(true));
            //QE Settings
            ...
            Menu.SubMenu("QEsettings").AddItem(new MenuItem("QEMR", "QE 最大范围 %").SetValue(new Slider(100, 0, 100)));
            Menu.SubMenu("QEsettings").AddItem(new MenuItem("UseQEC", "像鼠标方向QE").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));

            //R
            Menu.AddSubMenu(new Menu("R 设置", "Rsettings"));
            Menu.SubMenu("Rsettings").AddSubMenu(new Menu("能击杀不使用R", "DontRw"));
            Menu.SubMenu("Rsettings").SubMenu("DontRw").AddItem(new MenuItem("DontRwParam", "伤害来自").SetValue(new StringList(new[] { "All", "Either one", "None" }, 0)));
            ...
            Menu.SubMenu("Rsettings").AddSubMenu(new Menu("不对其使用R", "DontR"));
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != Player.Team))
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("DrawQEC", "QE 指示器").SetValue(new Circle(false, Color.FromArgb(100, 255, 0, 255))));
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Gank", "Gankable指示器").SetValue(true));
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("KillText", "击杀提示").SetValue(true));
