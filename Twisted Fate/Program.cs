...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("卡牌大师-崔斯特", "TwistedFate", true);

            var TargetSelectorMenu = new Menu("目标选择器", "Target Selector");
            SimpleTs.AddToMenu(TargetSelectorMenu);
            Config.AddSubMenu(TargetSelectorMenu);

            var SowMenu = new Menu("走砍选项", "Orbwalking");
            SOW = new Orbwalking.Orbwalker(SowMenu);
            Config.AddSubMenu(SowMenu);

            /* Q */
            var q = new Menu("Q - 技能", "Q");
            q.AddItem(new MenuItem("AutoQI", "自动Q禁锢 ").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "自动Q突进").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "自动Q").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W - 技能", "W");
            w.AddItem(
                new MenuItem("SelectYellow", "选黄牌").SetValue(new KeyBind("W".ToCharArray()[0],
                    KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectBlue", "选蓝牌").SetValue(new KeyBind("E".ToCharArray()[0], KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectRed", "选红牌").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(w);

            var r = new Menu("R - 技能", "R");
            r.AddItem(new MenuItem("AutoY", "开大招前自动选黄牌").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("其他选项", "Misc");
            misc.AddItem(new MenuItem("PingLH", "击杀提醒 (本地)").SetValue(true));
            ...
            /*Drawing*/
            var Drawings = new Menu("显示选项", "Drawings");
            ...
            Config.AddItem(new MenuItem("Combo", "连招快捷键").SetValue(new KeyBind(32, KeyBindType.Press)));
