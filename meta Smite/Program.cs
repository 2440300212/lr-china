...
namespace meta_Smite
{
    class Program
    {
        ...
        private static void Game_OnGameStart(EventArgs args)
        {
            ...
            setSmiteSlot();
            Config = new Menu("打野助手", "metaSmite", true);
            Config.AddItem(new MenuItem("Enabled", "自动惩戒").SetValue(new KeyBind("N".ToCharArray()[0], KeyBindType.Toggle, true)));
            Config.AddItem(new MenuItem("EnabledH", "按键惩戒").SetValue(new KeyBind("K".ToCharArray()[0], KeyBindType.Press)));
            Config.AddItem(new MenuItem("DrawStatus", "显示范围!").SetValue(true));
        ...
        {
            Config.AddSubMenu(new Menu("阵营", "Camps"));
            if(Game.MapId == GameMapId.TwistedTreeline)
            {
                Config.SubMenu("Camps").AddItem(new MenuItem("TT_Spiderboss", "大蜘蛛BOSS").SetValue(true));
                Config.SubMenu("Camps").AddItem(new MenuItem("TT_NGolem", "魔像").SetValue(true));
                Config.SubMenu("Camps").AddItem(new MenuItem("TT_NWolf", "狼").SetValue(true));
                Config.SubMenu("Camps").AddItem(new MenuItem("TT_NWraith", "幽灵").SetValue(true));
            }
			if (Game.MapId == (GameMapId)11)
			{
                //start by SKO
				Config.SubMenu("Camps").AddItem(new MenuItem("SRU_BaronSpawn", "纳什男爵").SetValue(true));
				Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Dragon", "小龙").SetValue(true));
				Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Blue", "蓝 Buff").SetValue(true));
				Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Red", "红 Buff").SetValue(true));
                //end
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Gromp", "蛤蟆").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Murkwolf", "狼").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Krug", "石头人").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Razorbeak", "怪鸟").SetValue(false));
