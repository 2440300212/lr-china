...
namespace LightningRyze
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
			...
			Config = new Menu("流浪法师-瑞兹", "Twilight Ryze", true);
			var targetSelectorMenu = new Menu("目标选择器", "Target Selector");
			SimpleTs.AddToMenu(targetSelectorMenu);
			Config.AddSubMenu(targetSelectorMenu);

            Menu orbwalker = new Menu("走砍选项", "orbwalker");
			...
			Config.SubMenu("Combo").AddItem(new MenuItem("TypeCombo", "").SetValue(new StringList(new[] {"混合连招","爆发连招","长连招"},0)));
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("HarassActive", "骚扰!").SetValue(new KeyBind("N".ToCharArray()[0], KeyBindType.Press)));
            ...
            Config.AddSubMenu(new Menu("漏洞破解", "Exploit"));
            Config.SubMenu("Exploit").AddItem(new MenuItem("UsePacket", "使用封包技能").SetValue(true));
            Config.SubMenu("Exploit").AddItem(new MenuItem("tearStack", "Q+W 双重撕裂效果 (测试)").SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Press)));
            ...
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("JR", "使用 R").SetValue(true));
            ...
            Config.SubMenu("Extra").AddItem(new MenuItem("HP", "使用天使HP%").SetValue(new Slider(20, 100, 0)));
            Config.SubMenu("Extra").AddItem(new MenuItem("UseWGap", "敌方近身自动W").SetValue(true));
            Config.SubMenu("Extra").AddItem(new MenuItem("AutoPoke", "自动骚扰").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Toggle)));
            Config.SubMenu("Extra").AddItem(new MenuItem("ManaFreeze", "骚扰蓝量%").SetValue(new Slider(40, 1, 100)));
            Config.SubMenu("Extra").AddItem(new MenuItem("WInterruptSpell", "使用W打断敌方").SetValue(true));
            
            Config.AddSubMenu(new Menu("地图破解", "MapHack"));
            Config.SubMenu("MapHack").AddItem(new MenuItem("TextColorMH", "文字颜色").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 0))));
            Config.SubMenu("MapHack").AddItem(new MenuItem("OutlineColorMH", "大纲颜色").SetValue(new Circle(true, Color.FromArgb(255, 0, 0, 0))));
            Config.SubMenu("MapHack").AddItem(new MenuItem("MapHack", "开启").SetValue(true));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("drawDamage", "显示计算伤害").SetValue(true));
